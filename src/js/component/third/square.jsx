    ...
    <>
      ...
      >
        {store.ships.map((ship, i) => {
          ...
          // else console.log(ship.name, row, col, 'square');//repite muchas veces todo // : sobrevolado? 'light' // sobrevolado? <span className='lightSquare'></span> :
          // return ship
        })}

        {board === store.PcBoard ? coord.name
          :
          coord === 0 ?  '' : coord.name
        } 
        {/* // : store.ships.map((ship, i) => { 
